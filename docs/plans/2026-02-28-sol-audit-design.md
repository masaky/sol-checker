# sol-audit Design Document

## Overview

Solidity スマートコントラクトの脆弱性を LLM で診断する CLI ツール。
個人 Solidity 開発者がデプロイ前に `sol-audit scan MyToken.sol` で即チェックできる。
...
```
$ sol-audit scan ./src/MyToken.sol
         │
         ▼
┌─────────────────────────┐
...
```bash
sol-audit scan <file>          # スキャン実行
sol-audit init                 # ~/.sol-audit/config.toml 生成
sol-audit --help
```
...
### Config File (~/.sol-audit/config.toml)
...
├── prompts/
│   └── audit-system.md       # システムプロンプト（外部ファイル化）
└── test/
    ├── fixtures/
