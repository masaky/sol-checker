...
- **Floating pragma — already-released range (reproducibility framing)**: When the entire allowed range consists of already-released compiler versions (e.g., `>=0.7.0 <0.9.0` where all 0.7.x and 0.8.x releases already exist), do NOT describe the risk as "future breaking changes" — that framing is misleading for a bounded, fully-released range. Instead, frame the risk as **reproducibility and determinism**: "Any 0.7.x or 0.8.x compiler may be selected, making the build non-deterministic. Pin to a specific tested version to ensure identical bytecode across environments." Reserve "future breaking changes" language only when the range's upper bound leaves room for compiler versions that have not yet been released (e.g., `>=0.8.0` with no upper ceiling, or a range that includes the current latest minor).
- **Floating pragma — effective compiler floor from version-gated syntax**: The declared pragma lower bound is NOT always the version range that can actually compile the file. Before citing specific selectable versions in the `description` or `impact`, scan the source for version-gated syntax/features and raise the effective floor to match the highest one used. Common gates: `type(T).max` / `type(T).min` require ≥0.6.8; custom errors (`error Foo();` + `revert Foo()`) require ≥0.8.4; `receive()` / virtual-override / `try`/`catch` require ≥0.6.0; checked-by-default arithmetic / `unchecked {}` requires ≥0.8.0; `pragma abicoder v2` as default requires ≥0.8.0. When a gated feature pushes the effective floor above the declared lower bound (e.g., `pragma solidity >=0.5.0;` but the file uses `type(uint128).max`, so it cannot compile below 0.6.8), do NOT cite versions below the effective floor as "selectable" examples (e.g., do NOT say "0.5.x vs 0.8.x may be chosen"). Cite only versions ≥ the effective floor, and note that the declared lower bound understates the true minimum. Additionally, when the audited file is a pure utility (library / pure-function math with no state, external calls, or storage), the "<0.8.0 lacks overflow protection affects callers" framing usually does NOT apply — verify whether the arithmetic can actually overflow in this code before asserting overflow impact; if the bit/index math is statically bounded, drop the overflow-impact claim and keep the finding to determinism/reproducibility.
- **Deprecated constructor visibility (version-specific)**: Detect any constructor declaration containing a `public` or `internal` keyword after the closing parenthesis of the parameter list (e.g., `constructor(uint a) BaseContract(a) public {}` or `constructor() internal {}`). Apply the following version-specific behavior:
  - **Solidity ≥0.8.0** (e.g., `^0.8.x`, pinned `0.8.x`, `>=0.8.0`): constructor visibility is **invalid syntax and a compile error** — the contract will NOT compile. The description MUST state: "constructor visibility is invalid syntax in Solidity ≥0.8.0 and causes a compile error." Report as **INFO** with title "Deprecated constructor visibility modifier".
  - **Solidity 0.7.x** (e.g., pinned `0.7.x`, `^0.7.0`): constructor visibility was deprecated in 0.7.0 — the compiler emits a **warning** but the code still compiles. The description MUST state: "constructor visibility was deprecated in Solidity 0.7.0; the compiler emits a warning." Report as **INFO**.
  - **Solidity ≤0.6.x**: `public`/`internal` constructors are valid and intentional — do NOT flag.
  Rationale: this is a compile-time issue that belongs in the code-quality category, not the security severity scale. Cite the constructor line. Fix: remove the visibility modifier.

- **SafeMath redundancy with unresolved imports**: When emitting a finding that `SafeMath` is redundant for Solidity ≥0.8.0, but the `SafeMath` source file appears in the "Unresolved import(s)" list, qualify the finding with: "Assuming standard SafeMath implementation — the import was not available for analysis; if this is a custom library, behavior cannot be verified." Do NOT suppress the finding; make the caveat explicit so reviewers know the implementation was not inspected.
- **SafeMath redundancy — call-site-aware fix**: Before writing the `fix` for a redundant-SafeMath finding, scan the source for actual invocations of SafeMath methods (`.add(`, `.sub(`, `.mul(`, `.div(`, `.mod(`). If any call site exists, "remove the `import` and `using` statement" alone is INVALID — it leaves the `.add()`/`.sub()`/etc. calls as undefined member references and the contract will NOT compile. The fix MUST instruct, in order: (1) replace each SafeMath method call with the equivalent native operator (`a.add(b)` → `a + b`, `a.sub(b)` → `a - b`, `a.mul(b)` → `a * b`, `a.div(b)` → `a / b`, `a.mod(b)` → `a % b`), citing the line numbers of the call sites; then (2) remove the `using SafeMath for ...` directive and the `import`. Only when there are zero call sites may the fix be the bare "remove the import and using statement." Do NOT assert "unnecessary gas overhead" or "equivalent to built-in" as fact when the SafeMath import is unresolved — keep that to a conditional design observation.
...
- **Deprecated modifier impact — verify branch semantics before writing `impact`**: When a deprecated modifier gates execution with a conditional (e.g., `if (msg.sender == admin) { _; } else { _fallback(); }`), read the modifier body to determine which branch executes the function's own logic and which branch delegates elsewhere — do NOT assume the privileged caller is the one redirected away. Example: OZ `TransparentUpgradeableProxy.ifAdmin()` runs the function body when `msg.sender == admin` and calls `_fallback()` only for non-admin callers; the actual selector-clash risk is a **non-admin** call being forwarded to the implementation using the admin function's calldata layout, not the admin being forwarded away from their own function. Writing "when the admin calls this selector, the modifier forwards to the implementation instead of executing the admin body" inverts the control flow — trace the `if`/`else` branches before describing the failure mode.
